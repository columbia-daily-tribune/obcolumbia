...
class Migration(DataMigration):
    ...
    depends_on = (
        ('db', '0022_auto__del_field_newsitem_block'),
        )
    ...
    def forwards(self, orm):
        ...
        schemas = orm['db.schema'].objects.filter(slug__in=restricted_slugs)
        model = orm['obcolumbia.schemarestriction']
        restriction, created = model.objects.get_or_create(subscriber_status='is_active')
        restriction.schemas = schemas
        restriction.save()
        # # Confirmed: it definitely gets saved here.
        # For some reason during test runs, it mysteriously disappears.
        #print model.objects.all()
        #from obcolumbia.models import SchemaRestriction
        #print SchemaRestriction.objects.all()
